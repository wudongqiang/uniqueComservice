...
public class RegServiceImpl implements RegService {
	...
	public HashMap<String, Object> reg(@ParamNoNull String dutyId,
			...
			String sourceCode, String hisUserId, String cardTypeId,
			Integer isBund) {
		...
		if (org.getHasRemoteReg() == null
			...
			try {
				syResult = synchronizedToOrg_Reg(friend, user,
						org.getHospitalUrl(), reg, duty, reg.getIsBund());
			...
			//就诊卡号
			if (syResult.containsKey("icCardId")) {
				reg.setIcCardId((String) syResult.get("icCardId"));
			}
			//排队序号
			if (syResult.containsKey("callNo")) {
				reg.setCallNo((String) syResult.get("callNo"));
			}

			log.info("判断是否退号");
			log.info(syResult.get("retCode").toString());
			if (syResult.containsKey("retCode")
					&& (syResult.get("retCode").toString().trim().equals("0") || syResult
	...
	public HashMap<String, Object> synchronizedToOrg_Reg(UserFriend userFriend,
			AmsUser user, String hospitalUrl, McReg reg, McDuty duty,
			Integer isBund) {
		...
		userOrg.setSex(user.getSex() == null ? null : user.getSex().toString());
		...
		reg4Org.setIsBund(isBund);
		PayWay payWay = orderDao.getPayWayById(reg.getPayWayId().toString());
		...
		if (payWay.getPayWayCode().equals("ZFB")) {
			...
			reg4Org.setPayWay("A1");
				...
				|| payWay.getPayWayCode().equals("WXZF_APP")) {
		...
		try {
			...
			try {
				...
				log.info("请求医院数据：" + JSONObject.fromObject(gparam));

				// 添加成功
				str = HttpUtil.sendPost(
						hospitalUrl
								+ "module=regService&method=addRegistration3",
						"{param:\""
			...
			if ((result.get("retCode").toString().equals("0") || result
				...
				if (obj.containsKey("retCode")
					...
					result.put("icCardId", obj.get("icCardId"));
					result.put("callNo", obj.get("callNo"));
	...
	/**
	 * 挂号医院同步接口 /以下代码为原来的备份代码
	 */
	public HashMap<String, Object> synchronizedToOrg_Reg_bak(
			UserFriend userFriend, AmsUser user, String hospitalUrl, McReg reg,
			McDuty duty, Integer isBund) {
		Org org = regDao.getOrgById(reg.getOrgId().toString());
		if (org.getOrgCode() != null
			...
			try {
				...
				log.info("去HIS前置接口新增用户:" + JSONObject.fromObject(gparam));
	...
	public HashMap<String, Object> synchronizedToOrg_UnReg(String hospitalUrl,
		...
		if (org.getOrgCode() != null
			...
			param.put("patientId", reg.getHisUserId());
			param.put("visitNo", reg.getAppointCode());
			param.put("ghf", String.valueOf(reg.getAmount()));
			...
			log.info("去HIS前置接口退号:" + JSONObject.fromObject(gparam));
			try {
				String str = HttpUtil.sendPost(
						hospitalUrl
	...
	@HttpMethod
	@Override
	public String checkRegistration(@ParamNoNull String orgId,
		...
		params.put("idCard", userFriend.getUserIdcard());
		params.put("userName", userFriend.getUserName());

		params.put("cardCode", "");
		params.put("safetyCode", "");
		List<McCard> cards = userDao.getCardByFriendId(friendId);
		for (McCard c : cards) {
			if (c.getCardTypeName() != null
					&& c.getCardTypeName().equals("就诊卡")) {
				params.put("cardCode", c.getCardNo());
				break;
			} else if (c.getCardTypeName() != null
					&& c.getCardTypeName().equals("医保卡")) {
				params.put("safetyCode", c.getCardNo());
				break;
			}
		}
		log.info("挂号前验证-请求参数：" + params);
		log.info("机构：" + org);
		String result = RemotUtils.getRemotData(org.getHospitalUrl(),
				"module=regService&method=checkRegistration", params);
